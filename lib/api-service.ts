...
export interface DetectionRecord {
  ...
  fileName: string;
  fileType: string;
  results: CodeDetectionResult[] | ImageDetectionResult[];
  createdAt: Date;
  userId?: string;
}
 ...
 * 检测图像中的敏感信息
 */
export async function detectImageSensitiveInfo(
  imageFile: File
): Promise<ApiResponse<ImageDetectionResult[]>> {
  try {
    // 导入现有的图像检测功能
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : '图像检测失败，请重试'
    };
  }
}
...
export async function saveDetectionRecord(
  record: Omit<DetectionRecord, 'id' | 'createdAt'>
): Promise<ApiResponse<DetectionRecord>> {
  try {
    // 这里可以集成数据库操作
    // 暂时返回模拟数据
 ...
 * 删除检测记录
 */
export async function deleteDetectionRecord(
  recordId: string
): Promise<ApiResponse<void>> {
  try {
    // 这里可以集成数据库删除操作
