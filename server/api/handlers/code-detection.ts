 ...
 * 处理代码检测请求
 */
export async function handleCodeDetection(request: NextRequest, operation: string) {
  try {
    ...
    if (!isCodeFile(fileName)) {
      return NextResponse.json(
        { error: '不支持的文件类型，请上传代码文件', success: false },
        { status: 400 }
      );
    }
    ...
    // 执行检测
    const detectionResult = await detectCodeSensitiveInfo({
      fileContent,
      fileName,
      fileType,
    });
